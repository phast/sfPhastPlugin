		...
		switch ($this->type) {
			...
			case 'file':
				...
				if('file' == $this->type && $this->getAttribute('render')){
					...
					$this->box->addSystemEvent('afterRender', "node.find('.$guid').html((function(){var value = box.data.phast_file_{$name};{$action}})());");
				}
				break;
			...
			case 'textedit':
				$output .= "<textarea name=\"{$name}\" style=\"{$style}\" class=\"phast-box-textedit {$class}\"></textarea>";
				break;
